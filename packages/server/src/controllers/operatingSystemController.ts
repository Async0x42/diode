...
export class OperatingSystemController {
  @Inject()
  private service!: OperatingSystemService;

  @Get('/')
  async getList(): Promise<OperatingSystem[]> {
    return this.service.findAll({}, { populate: columnsToPopulate });
  }

  @Get('/:id')
  async get(@PathParams('id') id: string, @Res() res: Res): Promise<OperatingSystem | undefined> {
    try {
  ...
  create(@BodyParams() user: OperatingSystem): Promise<OperatingSystem> {
    return this.service.create(user);
  }

  @Put('/:id')
  async update(@PathParams('id') id: string, @Res() res: Res, @BodyParams() @Any() payload: any): Promise<OperatingSystem | null | undefined> {
    try {
      const itemId = parseInt(id);
