...
export class NetworkController {
  ...
  @Get('/')
  async getList(): Promise<Network[]> {
    return this.service.findAll({}, { populate: columnsToPopulate });
  }

  @Get('/:id')
  async get(@PathParams('id') id: string, @Res() res: Res): Promise<Network | undefined> {
    try {
  ...
  create(@BodyParams() user: Network): Promise<Network> {
    return this.service.create(user);
  }

  @Put('/:id')
  async update(@PathParams('id') id: string, @Res() res: Res, @BodyParams() @Any() payload: any): Promise<Network | null | undefined> {
    try {
      const itemId = parseInt(id);
