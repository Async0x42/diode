...
import { SslCertificateService } from '../services';
import { SslCertificate } from '../entities';
import { Transactional } from '@tsed/mikro-orm';
import { Any, Delete, Put } from '@tsed/schema';

const columnsToPopulate = ['applications', 'servers'];

@Controller('/sslCertificates')
export class SslCertificateController {
  @Inject()
  private service!: SslCertificateService;

  @Get('/')
  async getList(): Promise<SslCertificate[]> {
    return this.service.findAll({}, { populate: columnsToPopulate });
  }

  @Get('/:id')
  async get(@PathParams('id') id: string, @Res() res: Res): Promise<SslCertificate | undefined> {
    try {
  ...
  create(@BodyParams() user: SslCertificate): Promise<SslCertificate> {
    return this.service.create(user);
  }

  @Put('/:id')
  async update(@PathParams('id') id: string, @Res() res: Res, @BodyParams() @Any() payload: any): Promise<SslCertificate | null | undefined> {
    try {
      const itemId = parseInt(id);
