...
import { BrdService } from '../services';
...
export class BrdController {
  @Inject()
  private service!: BrdService;

  @Get('/')
  async getList(): Promise<Brd[]> {
    return this.service.findAll({}, { populate: columnsToPopulate });
  }

  @Get('/:id')
  async get(@PathParams('id') id: string, @Res() res: Res): Promise<Brd | undefined> {
    try {
  ...
  create(@BodyParams() user: Brd): Promise<Brd> {
    return this.service.create(user);
  }

  @Put('/:id')
  async update(@PathParams('id') id: string, @Res() res: Res, @BodyParams() @Any() payload: any): Promise<Brd | null | undefined> {
    try {
      const itemId = parseInt(id);
