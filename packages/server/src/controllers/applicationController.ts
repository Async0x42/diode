...
export class ApplicationController {
  ...
  @Get('/')
  async getList(): Promise<Application[]> {
    return this.service.findAll({}, { populate: columnsToPopulate });
  }

  @Get('/:id')
  async get(@PathParams('id') id: string, @Res() res: Res): Promise<Application | undefined> {
    try {
  ...
  create(@BodyParams() user: Application): Promise<Application> {
    return this.service.create(user);
  }

  @Put('/:id')
  async update(@PathParams('id') id: string, @Res() res: Res, @BodyParams() @Any() payload: any): Promise<Application | null | undefined> {
    try {
      const itemId = parseInt(id);
