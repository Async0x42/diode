...
const columnsToPopulate = ['applications', 'servers', 'servers.environment', 'servers.location', 'servers.operatingSystem', 'servers.types'];
...
export class TicketController {
  ...
  @Get('/')
  async getList(): Promise<Ticket[]> {
    return this.service.findAll({}, { populate: columnsToPopulate });
  }

  @Get('/:id')
  async get(@PathParams('id') id: string, @Res() res: Res): Promise<Ticket | undefined> {
    try {
  ...
  create(@BodyParams() user: Ticket): Promise<Ticket> {
    return this.service.create(user);
  }

  @Put('/:id')
  async update(@PathParams('id') id: string, @Res() res: Res, @BodyParams() @Any() payload: any): Promise<Ticket | null | undefined> {
    try {
      const itemId = parseInt(id);
