...
export class ZoneController {
  ...
  @Get('/:id')
  async get(@PathParams('id') id: string, @Res() res: Res): Promise<Zone | undefined> {
    try {
  ...
  create(@BodyParams() user: Zone): Promise<Zone> {
    return this.service.create(user);
  }

  @Put('/:id')
  async update(@PathParams('id') id: string, @Res() res: Res, @BodyParams() @Any() payload: any): Promise<Zone | null | undefined> {
    try {
      const itemId = parseInt(id);
