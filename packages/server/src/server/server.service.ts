...
export const findAll = async (): Promise<Server[]> => (await DI.serverRepo.find({}, ['fqdns', 'applications'])) || [];

export const find = async (id: number): Promise<Server | null> => await DI.serverRepo.findOneOrFail({ id }, ['fqdns', 'applications']);
