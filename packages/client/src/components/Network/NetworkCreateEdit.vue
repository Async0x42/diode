...
const { onSubmit, onDelete } = useFormActions<INetwork>('/api/networks', 'networks', props.network);

const model = createFormModel<INetwork>(['name', 'shortName']);
const rules = {
  name: [
    {
      required: true,
...
assignDefaultsToForm(model, props.network);
...
<template>
  <n-page-header class="mx-8 mt-6" title="Network Information" />
  <n-form ref="formRef" :rules="rules" :model="model" class="mx-12">
    ...
    <div class="flex">
      <FormButtonDelete v-if="props.network" class="mr-3" @click="onDelete()" />
      <div class="flex"></div>
