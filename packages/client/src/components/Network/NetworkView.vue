...
<template>
  <div class="bg-white shadow overflow-hidden sm:rounded-lg">
    <div class="py-5 px-4 sm:px-6">
      ...
      <p class="mt-1 text-sm max-w-2xl text-gray-500">Network details and notes.</p>
    </div>
    <div class="border-t border-gray-200 py-5 px-4 sm:px-6">
      <dl class="grid gap-x-4 gap-y-8 grid-cols-1 sm:grid-cols-2">
        <div class="sm:col-span-1">
          ...
          <dd class="mt-1 text-sm text-gray-900">{{ props.network.name }}</dd>
          <dd class="mt-1 text-sm text-gray-700">{{ props.network.shortName }}</dd>
        </div>
      </dl>
    </div>
  </div>
