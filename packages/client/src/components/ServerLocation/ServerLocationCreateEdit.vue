...
const { onSubmit, onDelete } = useFormActions<IServerLocation>('/api/serverLocations', 'serverLocations', props.serverLocation);

const model = createFormModel<IServerLocation>(['name', 'shortName']);
const rules = {
  name: [
    {
      required: true,
...
assignDefaultsToForm(model, props.serverLocation);
...
<template>
  <n-page-header class="mx-8 mt-6" title="ServerLocation Information" />
  <n-form ref="formRef" :rules="rules" :model="model" class="mx-12">
    ...
    <div class="flex">
      <FormButtonDelete v-if="props.serverLocation" class="mr-3" @click="onDelete()" />
      <div class="flex"></div>
