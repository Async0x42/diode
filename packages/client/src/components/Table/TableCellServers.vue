...
<template>
  <n-td class="min-w-11.5 whitespace-nowrap">
    <router-link
      ...
      <ServerTooltip :server="server">
        <div :class="{ 'mb-2': props.servers.length > 1 && index !== props.servers.length - 1 }">
          <n-text tag="div" depth="1">
            <n-tag
              size="small"
              class="cursor-pointer whitespace-nowrap group-hover:text-teal-300"
            >{{ server?.name }}</n-tag>
          </n-text>
          <n-text tag="div" depth="3">
            ...
            >{{ server.location?.shortName || server.location?.name }}</n-tag>
            <n-tag
              size="small"
              class="cursor-pointer whitespace-nowrap group-hover:text-teal-300"
              v-if="server.environment"
            >{{ server.environment?.shortName || server.environment?.name }}</n-tag>
            <n-tag
              v-for="type in server.types"
              size="small"
              class="cursor-pointer whitespace-nowrap group-hover:text-teal-300"
            >{{ type?.shortName || type?.name }}</n-tag>
          </n-text>
        </div>
      </ServerTooltip>
    </router-link>
