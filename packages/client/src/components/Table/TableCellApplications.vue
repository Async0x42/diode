  ...
  <n-td>
    ...
    >
      <ApplicationTooltip :application="application">
        <n-tag v-if="useShortName && application?.shortName" size="small" type="info" class="mr-1 whitespace-nowrap hover:cursor-pointer hover:text-teal-300">
          {{ application?.shortName }}
        </n-tag>
        <n-tag v-else type="info" size="small" class="mr-1 hover:cursor-pointer">{{ application?.name }}</n-tag>
      </ApplicationTooltip>
    </router-link>
  </n-td>
</template>
