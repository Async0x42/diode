...
<template>
  <template v-if="!Array.isArray(props.servers)">
    <div class="group">
      <n-text v-if="showTitle" tag="div" depth="1" class="group-hover:text-teal-300">{{ props.servers.name }}</n-text>
      <n-text tag="div" depth="2" class="group-hover:text-teal-400">{{ props.servers.location?.shortName || props.servers.location?.name }}</n-text>
  ...
  <template v-else>
    <div v-for="server in props.servers" :key="server.id" class="mb-2 group">
      ...
      <n-text tag="div" depth="2" class="group-hover:text-teal-400">{{ server.location?.shortName || server.location?.name }}</n-text>
